 ...
 *
 * Async now — fetches site settings directly (this page is already a
 * Server Component, so it can call getSiteSettings() itself rather than
 * receiving it as a prop from the layout).
 */
export default async function ContactPage() {
  const settings = await getSiteSettings();

  const quickContacts = [
    { href: `https://wa.me/${settings.whatsappNumber}`, icon: MessageCircle, label: "WhatsApp Us", external: true },
    { href: toPhoneHref(settings.phone), icon: Phone, label: settings.phone, external: false },
    { href: `mailto:${settings.email}`, icon: Mail, label: settings.email, external: false },
  ];
      ...
      <div className="grid gap-14 lg:grid-cols-[1.3fr_1fr] lg:gap-20">
        <div className="order-1 flex flex-col gap-8 lg:order-2">
          ...
          <div className="flex items-center gap-5 border-t border-noble-line pt-6">
            <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="NOBLE on Instagram" className="text-noble-black transition-colors hover:text-noble-gold">
              <Instagram className="h-5 w-5" strokeWidth={1.5} />
            </a>
            <a href={settings.facebookUrl} target="_blank" rel="noopener noreferrer" aria-label="NOBLE on Facebook" className="text-noble-black transition-colors hover:text-noble-gold">
