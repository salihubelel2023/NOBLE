...
/**
 * Every known category is pre-rendered at build time by reading the
 * database. Add a category via /admin/categories and it's included on the
...
export default async function CategoryPage({ params, searchParams }: PageProps) {
  ...
  const { items, total } = filterAndSortProducts(allProducts, category, resolvedSearchParams);
  const brands = getAvailableBrands(allProducts, category.slug);
  const priceBounds = getPriceBounds(allProducts, category.slug);
