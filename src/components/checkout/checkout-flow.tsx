 ...
 * comes after the order, never required upfront. Mandatory account
 * creation before checkout is one of the most common causes of cart
 * abandonment; this flow never blocks the purchase behind it. The payment
 * step is UI-only (method selection, not card-number collection) since no
 * gateway is wired up yet — see /ARCHITECTURE.md and wireframe notes.
 */
export function CheckoutFlow() {
  ...
  function handlePlaceOrder() {
    const generatedNumber = `NB-${Math.floor(10000 + Math.random() * 89999)}`;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10 lg:px-8 lg:py-16">
      ...
      <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr] lg:gap-16">
        ...
        <OrderSummary lines={summaryLines} currency="USD" />
      </div>
    </div>
  );
}
